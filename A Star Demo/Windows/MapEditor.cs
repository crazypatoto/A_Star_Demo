...
namespace A_Star_Demo.Windows
{
    public partial class MapEditor : Form
    {
        ...
        private void MapEditor_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            (this.Tag as AStarDemo).SelectedEdgeNode1 = null;
            (this.Tag as AStarDemo).SelectedEdgeNode2 = null;
        }
        ...
        {
            (this.Tag as AStarDemo).comboBox_planningLayer.SelectedIndex = comboBox_constraintLayers.SelectedIndex;
        }

        private void button_addLayer_Click(object sender, EventArgs e)
        {
            var newLayer = new Map.ConstraintLayer((this.Tag as AStarDemo).CurrentMap, $"Layer {(this.Tag as AStarDemo).CurrentMap.ConstraintLayers.Count}");
            (this.Tag as AStarDemo).CurrentMap.ConstraintLayers.Add(newLayer);
            comboBox_constraintLayers.Items.Clear();
            foreach (var layer in (this.Tag as AStarDemo).CurrentMap.ConstraintLayers)
            {
        ...
        private void button_deleteLayer_Click(object sender, EventArgs e)
        {
            ...
            (this.Tag as AStarDemo).CurrentMap.ConstraintLayers.RemoveAt(comboBox_constraintLayers.SelectedIndex);
            comboBox_constraintLayers.Items.Clear();
            foreach (var layer in (this.Tag as AStarDemo).CurrentMap.ConstraintLayers)
            {
                comboBox_constraintLayers.Items.Add(layer.Name);
